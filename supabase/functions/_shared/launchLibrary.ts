type DatePrecision = "exact" | "day" | "month" | "quarter" | "half" | "year" | "range" | "tbd";
type MissionStatus = "planned" | "in-development" | "scheduled" | "active" | "completed" | "delayed" | "watch";
...
const normalizePrecision = (precision: LL2Precision, fallbackDate?: string | null): DatePrecision => {
  if (!precision) return fallbackDate ? "day" : "tbd";
  const value = (typeof precision === "string" ? precision : `${precision.name ?? ""} ${precision.abbrev ?? ""}`).toLowerCase();
  if (value.includes("second") || value.includes("minute") || value.includes("hour")) return "exact";
  if (value.includes("day") || value.includes("date")) return "day";
  ...
  if (value.includes("tbd") || value.includes("unknown")) return "tbd";
  return fallbackDate ? "day" : "tbd";
};

const normalizeStatus = (status?: LL2Status): MissionStatus => {
  const raw = `${status?.abbrev ?? ""} ${status?.name ?? ""}`.toLowerCase();
  if (raw.includes("success")) return "completed";
  if (raw.includes("hold") || raw.includes("delay") || raw.includes("failure") || raw.includes("partial")) return "delayed";
  if (raw.includes("go") || raw.includes("in flight")) return "scheduled";
  if (raw.includes("tbc") || raw.includes("tbd") || raw.includes("to be")) return "watch";
  return "watch";
};

const formatDateLabel = (date?: string | null, precision: DatePrecision = "tbd"): string => {
  if (!date) return "TBD";
